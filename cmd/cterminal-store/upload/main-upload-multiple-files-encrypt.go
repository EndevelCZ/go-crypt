...
var (
	...
	// }
	filePaths = []string{
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/1.jpg",
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/2.jpg",
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/3.jpg",
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/4.jpg",
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/5.jpg",
		"/Users/adamplansky/workspace/go/src/github.com/EndevelCZ/go-crypt/cmd/cterminal-store/upload/6.jpg",
	}
)
...
func main() {
	...
	for i, filePath := range filePaths {
		go func(cnt int, fp string, pubKeyStrings []string) {
			pbKs := []io.Reader{}
			for _, x := range pubKeyStrings {
				pbKs = append(pbKs, bytes.NewBufferString(x))
			}
			f, err := os.Open(fp)
			if err != nil {
			...
			defer f.Close()
			fmt.Println("starting to upload", cnt, fp)
			objname := fmt.Sprintf("%s-%d", objectName, cnt)
			resultChannel <- result{
				err:  gcs.UploadEncyptObject(f, objname, crypto.EncryptFn, pbKs...),
				name: objname,
			}
	...
	for i := range filePaths {
		...
		logrus.Infof("[%d.] encrypted and upload file OK! %s %s\n", i, res.name, time.Now())
		// fmt.Println(i, "encrypted file OK!")
		// fmt.Println(i, "upload file OK!")
		// fmt.Printf("%#v %s\n", res, res.err)
	}
	t := time.Now()
	elapsed := t.Sub(start)
